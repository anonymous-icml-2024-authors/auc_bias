...
parser.add_argument('--post_hoc_calibrate', action = 'store_true', help="post-hoc calibration using training set")
...
clf, pred_train, pred_val, pred_test = train_clf(args.algorithm, X_train, X_val, X_test, y_train, g_train, model_hparams = {i[5:]:hparams[i] for i in hparams if i.startswith('clf__')},
                                   cat_cols = TabularDataset.dataset_params[args.dataset].categorical_columns, post_hoc_calibrate=args.post_hoc_calibrate,
