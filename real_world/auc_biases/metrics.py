...
def prob_metrics(targets, preds, label_set, return_arrays=False):
    ...
    if len(set(targets)) == 2:
        ...
        res['AUNBC'] = net_benefit_curve_area(targets, preds)
        res['AUNBC_clip'] = net_benefit_curve_area(targets, preds, True)
...
def net_benefit_curve_area(y, y_pred, clip_neg = False):
    ...
    net_benefits = np.array([net_benefit(th) for th in thresholds])
    if clip_neg:
        net_benefits = np.clip(net_benefits, 0, None)
    auc = np.trapz(net_benefits, thresholds)
    return auc